...
const ContactsList = ({ user, setOpenChat }) => {
    ...
    const fetchUsers = async () => {
        //console.log(user)
        axios.get(`${baseURL}/users/${user._id}/contacts`, { withCrendentials: true}).then((res) => {
    ...
    const handleOpenChat = async (chatType, contact, room) => {
        if (chatType == "private") {
            await axios.get(`${baseURL}/users/chat/${user._id}/${contact._id}`).then(async(res) => {
                ...
                if (res.data.chat) {
                    setOpenChat({ chatId: res.data.chat._id, chatType: res.data.type, members: res.data.chat.members, title: contact.username })
                } else {
    ...
    return(
        <div id='contactsList'>
            <button onClick={() => setShowContacts(!showContact)}>
                Users
                {showContacts ?
                ...
                {showRooms && roomsList.length ? roomsList.map((room) => {
                    return (
                        <li onClick={() => handleOpenChat("room", null, room)} key={room._id}>
                            <p>{room.name}</p><p>{room.members.length} users</p></li>
                    )
